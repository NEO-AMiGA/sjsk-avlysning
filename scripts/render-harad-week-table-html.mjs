...
function buildStyles() {
  return [
    ...
    '    .weektable th{font-size:11px;font-weight:700;color:#111;text-align:left;padding:0 9px 5px 0;border-bottom:1px solid #ddd;white-space:nowrap}',
    '    .weektable td{padding:4px 9px 4px 0;border-bottom:1px solid #eee;vertical-align:top;white-space:nowrap}',
    ...
    '    .weektable .col-note{max-width:72px;white-space:normal}',
    styleEndMarker,
  ].join('\n');
}
...
function buildDayRow(week, day) {
  ...
  return [
    ...
    `            <td>${formatCellValue(day.restrictedTime)}</td>`,
    `            <td class="center risk">${renderDangerValue(day.dangerRange)}</td>`,
    `            <td class="center">${formatCellValue(day.otherActivity)}</td>`,
...
function buildSection(weeks) {
  ...
  return [
    ...
    '  <div class="tableinfo">',
    '    <p class="tableinfo-strong">⚠️ Det är LIVSFARLIGT att under ovan angivna klockslag beträda skjutfältet.</p>',
    '  </div>',
    '  <div class="tablewrap">',
    '    <table class="weektable">',
      '      <thead>',
        '        <tr>',
          ...
          '          <th>Dag</th>',
          '          <th>Datum</th>',
          '          <th class="col-focus">Förbudstid</th>',
          '          <th class="center col-focus">Risk över<br>banor</th>',
          '          <th class="center">Annan<br>verksamhet</th>',
          '          <th class="col-note">Anm.</th>',
        '        </tr>',
      '      </thead>',
      '      <tbody>',
    rows,
